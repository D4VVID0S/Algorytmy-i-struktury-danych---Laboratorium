...
void init(Graph* g, int n)
{
  ...
  // a - startVertex
  // b - endVertex
  g->n = n; //przypisanie do struktury graf ilosci wierzcholkow = 6 czyli (0, 5)
  g->adj = calloc(n, sizeof(bool*)); //alokacja pamieci w rozmiarze ilosci wierzcholkow
  for (k = 0; k < n; ++k)
  {
...
void addEdge(Graph* g, int a, int b)
{
  ...
  {
    g->adj[a][b] = true;
    // g->adj[b][a] = true; // skomentowane poniewaz ma to byc graf skierowany
  }
  else
  {
...
void dfs(Graph* g, int a)
{
  ...
  int k;
  // temp = podliczanie odwiedzonych wierzcholkow
  int temp = 0;
  for (k = 0; k < g->n; ++k)
      visited[k] = false;
  ...
  while (StanStosu() == OK)
  {
    ...
    if (b == -1)
    { // jezeli nie ma takiego wierzcholka,
      pop();
    }
    else
    {
